...
private class MoveDownAction extends AbstractAction{
  public void actionPerformed(ActionEvent e) {
	 ...
	 */
	private void setImageLink(SHTMLDocument doc, Element e, String href, String className, String linkImage,
	                          Dimension size) {
		...
		try {
			...
			set.addAttribute(HTML.Attribute.SRC, Util.getRelativePath(new File(doc.getBase().getFile()), new File(
			    linkImage)));
	...
	void moveTableColumnLeft() {
		...
		try {
			...
			int finalCaretPosition = tableRow.getElement(indexOfCellInRow - 1).getStartOffset()
	...
	void moveTableColumnRight() {
		...
		try {
			...
			int finalCaretPosition = getCaretPosition() + cellToTheRight.getEndOffset()
	...
	private void pasteHTML(final HTMLText pastedHTMLText, int position) throws Exception {
		...
		if (position == paragraphElement.getStartOffset()) {
			...
			if (caretWithinTableCell() && pasteHtmlTextModified.matches("(?ims).*<table.*")) {
				...
				String strippedHTMLText = pasteHtmlTextModified.replaceAll("(?ims).*<table.*?>", "").replaceAll(
				    "(?ims)</table.*?>.*", "");
				Element cellElement = getCurrentTableCell();
		...
		String newHtml = pastedHTMLText.splitPaste(sDocument, characterElement, paragraphElement, position,
	...
	private class ListManager {
		...
		private boolean switchOn(final String listTag, final AttributeSet attributeSet, final boolean forceOff,
		                         int start, int end, Element parentElement) throws SwitchListException {
		...
		private void increaseIndent(boolean includingSubitems) {
			...
			try {
				...
				String newContent = writer.getWrittenString().replaceAll("(?ims)</ul>\\s*<ul[^>]*>", "").replaceAll(
				    "(?ims)</ol>\\s*<ol[^>]*>", "");
		...
		private void writeListItemForDecreaseIndent(SHTMLWriter writer, Element listItem, boolean includingSubitems,
		...
		private void newListItem() {
			...
			try {
				...
				if (so != eo) {
					...
					if (caretPosition < eo - 1) {
						SHTMLWriter htmlEndWriter = new SHTMLWriter(stringWriter, doc, caretPosition, eo
						        - caretPosition);
	...
	public TransferHandler getTransferHandler() {
		...
		class LocalTransferHandler extends TransferHandler {
				...
				try {
					if (transferable.isDataFlavorSupported(htmlTextDataFlavor)) {
						...
						if (transferable.isDataFlavorSupported(htmlFlavor)
							...
							htmlContent = htmlContent.replaceAll("(?ims).*<body.*?>\\s*", "").replaceAll(
							    "(?ims)\\s*</body.*", "").replaceAll("<!--StartFragment-->", "").replaceAll(
							    "<!--EndFragment-->", "");
			...
			private boolean importExternalData(JComponent comp, final Transferable t) throws ClassNotFoundException,
	...
	public void applyAttributes(AttributeSet attributeSet, boolean applyToCompleteParagraph, boolean replace) {
		...
		if (applyToCompleteParagraph) {
			...
			if (selectionEnd != selectionStart) {
				...
				MutableAttributeSet inputAttributes = ((SHTMLEditorKit) getEditorKit()).getInputAttributes();
